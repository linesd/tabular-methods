...
def test_gridworld():
    ...
    num_cols = 12
    num_rows = 9
    obstructions = np.array([[8,6],[7,6],[6,6],[5,6],[3,6],[3,7],[3,8],[3,9]])
    ...
    # run tests
    assert np.any(model.R == grid_world['R'][0][0][:,0].reshape(-1,1))
    assert np.any(model.P[:,:,0] == grid_world['P'][0][0][:,:,0])
    assert np.any(model.P[:,:,0] == grid_world['P'][0][0][:,:,1])
    assert np.any(model.P[:,:,0] == grid_world['P'][0][0][:,:,2])
    assert np.any(model.P[:,:,0] == grid_world['P'][0][0][:,:,3])
